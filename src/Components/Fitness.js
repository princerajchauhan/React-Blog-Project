...
const Fitness = () => {
    ...
    useEffect(() => {
        document.title = `Blog Machine - Fitness`
    })

    const [topthree] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Fitness').slice(0, 3))

    const [topContext] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Fitness').slice(0, 10))

    const [letfTop] = useState(context.sort(() => Math.random() - Math.random()).filter(item => item.category === 'Fitness').slice(0, 5))
